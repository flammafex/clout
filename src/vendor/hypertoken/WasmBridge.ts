...
export async function loadWasm(): Promise<HyperTokenWasm> {
  ...
  wasmLoadPromise = (async () => {
    try {
      ...
      const wasmJsPath = join(__dirname, 'pkg', 'hypertoken_core.js');
      const wasmBinaryPath = join(__dirname, 'pkg', 'hypertoken_core_bg.wasm');

      const wasmImport = await import(wasmJsPath);
      const wasmBinary = readFileSync(wasmBinaryPath);

      if (wasmImport.initSync && typeof wasmImport.initSync === 'function') {
        wasmImport.initSync(wasmBinary);
      } else if (wasmImport.default && typeof wasmImport.default === 'function') {
